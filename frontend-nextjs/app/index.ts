...
import {
  MPL_TOKEN_METADATA_PROGRAM_ID as METADATA_ID,
  TokenStandard,
...
export class NftBurnBridging {
  ...
  constructor(
    ...
    this.program = new Program<NftBurnBridgingTypes>(IDL as any, {
      connection,
    });
    ...
    const metaplexClusterToWormholeNetwork = (cluster: Cluster) => {
      ...
      throw Error(
        `Unsupported cluster ${cluster}, please specify wormhole program id manually`
      );
    };
    this.wormholeId = new PublicKey(
      wormholeId ??
        CONTRACTS[metaplexClusterToWormholeNetwork(this.metaplex.cluster)]
          .solana.core
    );
  }
  ...
  async getInstance(mustBeInitialized = true) {
    ...
    const isInitialized =
      !!data && data.collectionMint.equals(this.collectionMint);
  ...
  getMessageAccountAddress(nftMint: PublicKeyInitData): PublicKey {
    return PublicKey.findProgramAddressSync(
  ...
  async isNftWhitelisted(
    nftTokenOrTokenId: PublicKey | number
  ): Promise<boolean> {
    const instance = await this.getInstance();
    if (instance.whitelistSize === 0) return true;
    const tokenId =
      typeof nftTokenOrTokenId === "number"
        ? nftTokenOrTokenId
  ...
  async createInitializeInstruction(
    ...
    const collectionNft = await this.metaplex
      .nfts()
      .findByMint({ mintAddress: this.collectionMint });

    return this.program.methods
      .initialize(whitelistSize)
      .accountsStrict({
  ...
  async createWhitelistBulkInstructions(
    ...
    return Promise.all(
      range(chunks).map((chunk) => {
        const whitelistSlice = whitelist.slice(
          chunk * chunkSize,
          (chunk + 1) * chunkSize
        );
        const bytes = range(Math.ceil(whitelistSlice.length / 8)).map(
          (byte) => {
            let byteValue = 0;
            for (
              let bit = 0;
              bit < 8 && byte * 8 + bit < whitelistSlice.length;
              ++bit
            )
        ...
        return this.program.methods
          .whitelistBulk(chunk * whitelistBytes, Buffer.from(bytes))
          .accountsStrict({
            instance: instance.address,
            authority,
          })
  ...
  async createSendAndBurnInstruction(
    ...
    const instance = await this.getInstance();
    if (instance.isPaused) throw Error("NftBurnBridging is paused");
    ...
    const evmRecipientArrayified = Array.from(
      ethers.utils.zeroPad(evmRecipient, 20)
    );
    ...
    const tokenRecord =
      nft.tokenStandard === TokenStandard.ProgrammableNonFungible
        ? this.metaplex
            .nfts()
            .pdas()
            .tokenRecord({ mint: nft.mint.address, token: nftToken })
    ...
    return this.program.methods
      .burnAndSend(evmRecipientArrayified)
      .accountsStrict({
        instance: instance.address,
        payer,
        ...
        collectionMeta: this.metaplex
          .nfts()
          .pdas()
          .metadata({ mint: this.collectionMint }),
        tokenRecord,
  ...
  private async getAndCheckNft(nftToken: PublicKey, loadJsonMetadata = false) {
    const nft = await this.metaplex
      .nfts()
      .findByToken({ token: nftToken, loadJsonMetadata });
